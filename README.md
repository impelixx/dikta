...
| **4 модели GigaAM v3** | CTC и Transducer, с пунктуацией и без — переключаются на лету из трея, с реальными замерами скорости |
...
Используются готовые ONNX-экспорты GigaAM v3 из [sherpa-onnx model
zoo](https://github.com/k2-fsa/sherpa-onnx/releases/tag/asr-models) — конверсия
из PyTorch не нужна, модель сразу готова к работе после скачивания.
...
| Модель | Тип | Пунктуация | Скорость (RTF) | Загрузка |
|---|---|---|---|---|
| GigaAM-CTC v3 | CTC | нет | ×12 к реальному времени | ~0.9с |
| GigaAM-CTC v3 + пунктуация | CTC | да | ×12 к реальному времени | ~0.9с |
| GigaAM-Transducer v3 | Transducer | нет | ×12 к реальному времени | ~2.3с |
| GigaAM-Transducer v3 + пунктуация | Transducer | да | ×12 к реальному времени | ~2.0с |
